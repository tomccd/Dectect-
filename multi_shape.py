...
for x in range(len(contours)):
    ...
    #--Tìm đường viền có xấp xỉ với đường viền của vật thể gốc
    ...
    if len_approx==4:
        ...
        condition_diagonal_2 = np.abs(AC_distance-diagonal) <= 2
        ...
        if np.logical_and(condition_diagonal_1,condition_diagonal_2) == True:
            ...
            if (np.abs(AB_distance-AD_distance) <=2) == True:
            ...
            condition_distance_parallelogram_1 = np.abs(AB_distance-CD_distance) <=2
            condition_distance_parallelogram_2 = np.abs(BC_distance-AD_distance) <=2
...
print(f'Number of Square : {count_square}')
print(f'Number of Rectangle : {count_rectangle}')
print(f'Number of Trapez : {count_trapez}')
print(f'Number of Parallelogram : {count_Parallelogram}')
cv.imshow('bla',arr)
